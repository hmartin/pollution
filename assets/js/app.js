...
$(document).ready(function () {
    ...
    var sliderCo2 = $("#slider-co2").on("input change", function() { updateAllValues(); });
    var sliderKm = $("#slider-km").on("input change", function() { updateAllValues(); });

    sliderCo2.val($("#car").val());
    updateAllValues();

    $("#car").on("change", function () {
        sliderCo2.val($(this).val());
    });

    function updateAllValues() {
        $('#number-co2').html(sliderCo2.val());
        $('#number-km').html(sliderKm.val());
        var annualCo2 = Math.round(sliderCo2.val()*sliderKm.val()/1000);
        var annualTrees = Math.round(annualCo2/10);
        // 1T co2 <=> 100 arbres
        $('#number-kg-co2-year').html(annualCo2);
        $('#number-tree').html(annualTrees);
        if (!isNaN(annualTrees)) {
            $('.icons-trees').html('');
            for (var i = 0; i < annualTrees; i++) {
                var iconTree = '<i class="fas fa-tree"></i>';
                $('.icons-trees').append(iconTree);
            }
        }
    }
});
